/*
 * $Id: as_crypt.c,v 1.7 2004/09/02 16:31:48 mkern Exp $
 *
...
/* Not sure how big this table actually is. For our current purposes 256 byte
...
as_uint8 *as_cipher_nonce (ASCipher *cipher, as_uint8 guid[16])
{
	...
	/* testing */
	memcpy (guid, "\x6C\xA5\x4A\x84\x1F\x80\x2F\xDB\x19\x61\x87\x6C\xC4\xAA\xC5\xE4", 16);
	cipher->session_seed_8 = 0x3B;
	cipher->session_seed_16 = 0x0607;
...
for (i = 0; i<16; i++)
	printf ("%02X ", guid[i]);
printf ("\n");
printf ("seed_8: %02X, seed_16: %04X\n", cipher->session_seed_8, cipher->session_seed_16);

	/* prepare GUID */
	memcpy (gbuf, guid, 16);

	gbuf[0] = (gbuf[1] ^ gbuf[11]) + 0x431F;

	for (i = 0; i < 16; i++)
		gbuf[i] ^= table_4[16 - 1 - i];
	memcpy (gbuf + 16, gbuf, 16);

	for (i = 0; i < 32; i++)
		gbuf[i] ^= table_4[32 - 1 - i];

	for (i = 0; i < 32; i++)
		gbuf[i] ^= table_4[32 - 1 - i];

	for (i = 0; i < 32; i++)
		gbuf[i] ^= table_4[i + 1] + table_1[i + 1];

	/* sha1 it five times */
	as_sha1_init (&sha1_state);
	for (i = 0; i < 5; i++)
		as_sha1_update (&sha1_state, gbuf, 32);
	as_sha1_final (&sha1_state, gbuf);

printf ("mangeled GUID: ");
for (i = 0; i<20; i++)
	printf ("%02X ", gbuf[i]);
printf ("\n");

	if (!(nonce = malloc (sizeof (as_uint8) * 24)))
		return NULL;
	...
	token = hash_lowered_token (gbuf, 20);

printf ("key: %04X, guid_token: %04X\n", key, token);

	/* assemble binary string */
	nonce[0] = (key       ) & 0xFF;
	nonce[1] = (key   >> 8) & 0xFF;
	nonce[2] = (token     ) & 0xFF;
	nonce[3] = (token >> 8) & 0xFF;
	memcpy (nonce + 4, gbuf, 20);

	/* sha1 it six times */
	as_sha1_init (&sha1_state);
	for (i = 0; i < 6; i++)
		as_sha1_update (&sha1_state, nonce, 24);
